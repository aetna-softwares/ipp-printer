...
function getPrinterAttributes (printer, req, res) {
  ...
  var attributes = printer.attributes(requested, req)
...
function getJobs (printer, req, res) {
  ...
  var jobs = states
    ? printer.getJobs(attributes).filter(function (job) { return ~states.indexOf(job.state) })
    : printer.getJobs(attributes)
